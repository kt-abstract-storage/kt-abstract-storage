...
class TruncatedStream(
    ...
    override fun write(buffer: ByteArray, offset: Int, count: Int) {
        ...
        if (count.toLong() > remainingCapacity) {
            throw StorageIOException("Write exceeds the truncated stream length.")
        }
    ...
    private fun ensureOpen() {
        if (closed) {
            throw StorageIOException("Stream is closed")
        }
    }

    private fun ensureReadable() {
        if (!stream.canRead) {
            throw StorageIOException("Stream does not support reading")
        }
    }

    private fun ensureWritable() {
        if (!stream.canWrite) {
            throw StorageIOException("Stream does not support writing")
        }
    }
